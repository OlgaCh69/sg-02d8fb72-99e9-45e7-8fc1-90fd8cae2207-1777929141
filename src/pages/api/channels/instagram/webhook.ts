        ...
        for (const entry of body.entry) {
          for (const messaging of entry.messaging || []) {
            if (messaging.message) {
              await handleInstagramMessage(messaging, settings);
            }
          }
        }
      }
...
async function handleInstagramMessage(messaging: any, settings: any) {
  ...
  let { data: conversation } = await supabase
    .from("conversations")
    .select("*")
    .eq("visitor_id", `instagram_${senderId}`)
    .eq("status", "active")
    .single();
  ...
  if (!conversation) {
    const { data: newConversation } = await supabase
      .from("conversations")
      .insert({
        visitor_id: `instagram_${senderId}`,
        session_id: `instagram_session_${Date.now()}`,
        channel: "instagram",
        status: "active",
